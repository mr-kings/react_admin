...
class CustomUpload extends Component {
	...
	handleBeforeUpload = (file, fileList) => {
		console.info('handleBeforeUpload file', file);
		console.info('handleBeforeUpload fileList', fileList);
		...
		if (!(isPic || isWord || isPDF)) {
			message.error(
				'只能上传JPG 、JPEG 、GIF、 PNG格式的图片或PDF、DOC、DOCX格式的文件~'
			);
		}
	...
	// 上传文件改变状态是
	handleChange = info => {
		console.info('handleChange info', info);
		let { fileList } = info;

		if (fileList.length > 10) {
			message.error('上传超出了限制');
		} else {
			console.info('正常上传', info);
	...
	handleCustomResquest = info => {
		console.info('handleCustomResquest info', info);
	};

	// 预览回调
	handlePreview = file => {
		console.info('handlePreview file', file);
	};

	// 删除回调
	handleRemove = file => {
		console.info('handleRemove file', file);
	};
	...
	render() {
		...
		const props = {
			name: 'files',
			accept:
				'image/*,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document', // 上传支持的格式
			action:
				'https://gdpo-test.yun.city.pingan.com/gdyqpt/api/obsInfo/upload/file', // 文件上传服务器
					...
					{fileList.map(item => (
						<div className="uploadItem" key={item.uid}>
							...
							<div className="itemRemove">
								{item.status === 'error' ? (
									<Icon
										className="error itemRemoveIcon"
										type="delete"
										title="删除该文件"
